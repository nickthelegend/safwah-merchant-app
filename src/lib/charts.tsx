...
export function LineChart({ data, color = INK, height = 200 }: { data: Pt[]; color?: string; height?: number }) {
  ...
  return (
    <svg viewBox={`0 0 ${w} ${h}`} style={{ width: "100%", height: "auto" }}>
      ...
      {[0, 0.5, 1].map((g) => <line key={g} x1={pad} x2={w - pad} y1={28 + g * (h - 56)} y2={28 + g * (h - 56)} stroke={GRID} />)}
      <path d={area} fill="url(#lg)" />
      <path d={line} stroke={color} strokeWidth={2.5} fill="none" strokeLinejoin="round" strokeLinecap="round" />
      {data.map((d, i) => <circle key={i} cx={px(i)} cy={py(d.y)} r={i === data.length - 1 ? 4.5 : 2.5} fill={i === data.length - 1 ? color : "#ffffff"} stroke={color} strokeWidth={1.5} />)}
      {data.map((d, i) => <text key={i} x={px(i)} y={h - 8} fill={AXIS} fontSize="10" textAnchor="middle" fontFamily="var(--font-mono)">{d.x}</text>)}
    </svg>
  );
}
...
export function Doughnut({ data, centerValue, centerLabel }: { data: Slice[]; centerValue?: string; centerLabel?: string }) {
  ...
  const offsets = data.map((_, i) => data.slice(0, i).reduce((sum, d) => sum + d.value, 0) / total);
  return (
    <div style={{ display: "flex", gap: 24, alignItems: "center", flexWrap: "wrap" }}>
      <div style={{ position: "relative", width: size, height: size }}>
        <svg width={size} height={size}>
          <g transform={`rotate(-90 ${cx} ${cy})`}>
            <circle cx={cx} cy={cy} r={r} stroke="#f0f0f2" strokeWidth={stroke} fill="none" />
            {data.map((d, i) => {
              const frac = d.value / total, dash = frac * C, off = -offsets[i] * C;
